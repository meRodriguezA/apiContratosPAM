...
from src.config import DEFAULT_TEMPLATE_FILENAME_BAJA_REIAC

class TemplateInstallerBajaReiac:
    def __init__(self, paths: AppPaths, logger: Logger = None):
        self.paths = paths
        self.logger = logger or Logger()

    def ensure_external_template(self) -> str:
        dst = os.path.join(self.paths.external_templates_dir, DEFAULT_TEMPLATE_FILENAME_BAJA_REIAC)

        if os.path.exists(dst):
            return dst

        src = self.paths.get_bundled_template_contrato()
